...
export default class Favourite extends Component {
    ...
    handleDelete=(id)=>{
        let newarr=[];
        newarr=this.state.movies.filter((moviesObj)=>moviesObj.id!=id)
        this.setState({
            movies:[...newarr]
        })
        ...
        <div className='row'>
            ...
            <div className='col-lg-9 favourites-table col-sm-12'>
                <div className='row'>
                    ...
                    <input type="number" className='input-group-number col' placeholder='Rows Count'value={this.state.limit} onChange={(e)=>this.setState({limit:e.target.value})}/>
                </div>
                <div className='row'>
                    <table class="table">
                        <thead>
                            ...
                            <th scope="col"><i class="fa-solid fa-sort-up" onClick={this.sortRatingsDesc}/>Ratings<i class="fa-solid fa-sort-down"onClick={this.sortRatingsAsc}/></th> 
                            <th></th>
                            </tr>
                        </thead>
                        <tbody>
                            {
                               filterarr.map((movieObj)=>(
                                    <tr>
                                    <td> <img src={`https://image.tmdb.org/t/p/original${movieObj.backdrop_path}`} alt={movieObj.title} style={{width:'7rem'}} />{movieObj.original_title}</td>
